...
if(!empty($_REQUEST["request"])){
	...
	elseif ($request == 'filter') {
		Filter($_REQUEST['filter'],$_REQUEST['user']);
	}	
	else{
		echo 'wrong';
	}
...
function homePage($x){
	...
	<div id="grid" class = "grid">
		...
		<div id="box2">
			<div id="grid3">
				<h1>Issues</h1>
				<button type="button" class= "button" style="background-color: #05c46b;color: white;" onclick="newIssue(<?php echo $x ?>)">Create New Issue</button>
			</div>
			<div id="grid2">
				<p style="font-family:'Calibri';font-size: 18px; ">Filter by:</p>
				<button type="button" onclick="Filter('All', <?php echo $x ?>)">All</button>
				<button type="button" onclick="Filter('OPEN', <?php echo $x ?>)">OPEN</button>
				<button type="button" onclick="Filter('userTix', <?php echo $x ?>)">MY TICKETS</button>
			</div>
			<table class="table">
				<thead class="head">
					<tr>
...
function Filter($x,$y){
	...
	if ($x == 'userTix' ){ 
		...
		$stmt = $conn->prepare("SELECT firstname,lastname FROM Users WHERE id = '$y' ");
		$stmt->execute();
		$results = $stmt->fetchAll(PDO::FETCH_ASSOC);

		$name = $results[0]['firstname'] . " " . $results[0]['lastname'];

		$stmt = $conn->prepare("SELECT * FROM Issues WHERE created_by = '$name' ");
		$stmt->execute();
		$results = $stmt->fetchAll(PDO::FETCH_ASSOC);  ?>
	...
	<?php
		...
		<div id="grid" class = "grid">
			...
			<div id="box2">
				<div id="grid3">
					<h1>Issues</h1>
					<button type="button" class= "button" style="background-color: #05c46b;color: white;" onclick="newIssue(<?php echo $y ?>)">Create New Issue</button>
				</div>
				<div id="grid2">
					<p style="font-family:'Calibri';font-size: 18px; ">Filter by:</p>
					<button type="button" onclick="Filter('All', <?php echo $y ?>)">All</button>
					<button type="button" onclick="Filter('OPEN', <?php echo $y ?>)">OPEN</button>
					<button type="button" onclick="Filter('userTix', <?php echo $y ?>)">MY TICKETS</button>
				</div>
				<table class="table">
					<thead class="head">
						<tr>
